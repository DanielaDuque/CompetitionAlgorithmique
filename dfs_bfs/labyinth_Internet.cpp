...
const int h[] = {1, -1, 0, 0}, v[] = {0, 0, 1, -1}, p[]= {'D', 'U', 'R', 'L'};

bool vis[1000][1000];
char c, par[1000][1000], ans[1000000];
int N, M, sx, sy, ex, ey, dist[1000][1000];

// To us a BFS, we need a queue
// First in, first out
queue<pii> Q;
    ...
    while(!Q.empty()){
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(0 <= dx && dx < N && 0 <= dy && dy < M && !vis[dx][dy]){
                // to recreate the path, we need to store the parent
                par[dx][dy] = p[i]; 
                // To get the distance, we need to store the distance
